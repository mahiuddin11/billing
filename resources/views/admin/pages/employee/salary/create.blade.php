                    ...
                    <form action="{{ $store_url ?? '#' }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-1">
                                ...
                                <select class="select2 form-control select2-lg" id="employee_id" aria-label=".select2-lg example"
                                    name="employee_id">
                                    <option selected disabled>Select employee</option>
                                    @foreach ($employees as $employee)
                                    <option salary="{{$employee->salary}}" value="{{$employee->id}}">{{$employee->name}}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-6 mb-1">
                            ...
                            <div class="col-md-6 mb-1">
                                ...
                                <input type="number" class="form-control input-rounded" id="paid_salary" name="paid_salary"
                            ...
                            <div class="col-md-6 mb-1">
                                <label for="">Payment Type</label>
                                 <select class="select2 form-control select2-lg" aria-label=".select2-lg example" name="payment_id">
                                     <option selected disabled>Select Payment Type</option>
                                     @foreach ($paymentMethods as $paymentMethod)
                                     <option value="{{$paymentMethod->id}}">{{$paymentMethod->account_name}}</option>
                                     @endforeach
                                 </select>
                            </div>
                            <div class="col-md-6 mb-1">
...
<script>
    $('#employee_id').on('change',function(){
        let salary = $(this).find(':selected').attr('salary');
